...
class RecModel(object):
    ...
    def modelCompile(self, model):
        model.summary()
        model.compile(loss='categorical_crossentropy', optimizer='adam', metrics=[top_10_CCE])
...
class RecommendModel(object):
    ...
    def build(self):
        ...
        # out=Concatenate(axis=-1)([attention_out,self_out])
        out = Dense(self.n_movies, activation='softmax', name='out')(attention_out)
        self = Model(input=inputs, output=out)

    def modelCompile(self):
        self.summary()
        self.compile(loss='categorical_crossentropy', optimizer='adam', metrics=[top_10_CCE])

    def train(self, gen_train, gen_val, scheduler):
        start_time = time.time()
        ...
        # call_back_list=[early_stopping,checkpoint]
        call_back_list = [checkpoint, reduce_lr, reduce_lr_on_pl]
        self.fit_generator(generator=gen_train, epochs=self.train_epochs, steps_per_epoch=self.n_train_user,
                                 validation_data=gen_val, validation_steps=self.n_val_user, callbacks=call_back_list)
        # finalmodel.fit_generator(generator=gen_data,epochs=train_epochs,steps_per_epoch=n_train_user)
        end_time = time.time()
        print('total time:%f' % (end_time - start_time))
